...
export abstract class HasRelationship<D extends ModelData> extends QueryBuilder<D> {
	protected hasMany<T extends Model>(relation: { new (): T }, name?: string): HasMany<T, D> {
		return new HasMany<T, D>(new relation(), this as unknown as Model, name);
	}

	protected hasOne<T extends Model>(relation: { new (): T }, name?: string): HasOne<T, D> {
		return new HasOne<T, D>(new relation(), this as unknown as Model, name);
	}

	protected belongsTo<T extends Model>(parent: { new (): T }, name?: string): BelongsTo<T, D> {
